...
func configDefault(config ...Config) Config {
	...
	if cfg.RequestAUTH == nil {
		...
		cfg.RequestAUTH = func(c *fiber.Ctx) (*DeserializeUser, error) {
			...
			host_auth, _ := viper.Get("AUTH_HOST").(string)

			host_path, _ := viper.Get("AUTH_PATH_AUTH").(string)

			host_auth_path := fmt.Sprintf("%s%s", host_auth, host_path)

			req, err := http.NewRequest(http.MethodGet, host_auth_path, nil)
			if err != nil {
